...
if (
	$cmsmasters_option['medical-clinic' . '_profile_post_cat'] || 
	$cmsmasters_option['medical-clinic' . '_profile_post_comment'] || 
	(
...
<article id="post-<?php the_ID(); ?>" <?php post_class('cmsmasters_open_profile'); ?>>
	...
	if ($cmsmasters_profile_title == 'true') {
		echo '<header class="cmsmasters_profile_header entry-header">';
			medical_clinic_profile_title_nolink(get_the_ID(), 'h1', $cmsmasters_profile_subtitle, 'h5');
		echo '</header>';
	}
	
	echo '<div class="cmsmasters_open_profile_inner">';
			...
			echo '<div class="profile_sidebar">';
				...
				if ($profile_details == 'true') {
					echo '<div class="profile_details entry-meta">' . 
						...
						medical_clinic_get_profile_likes('post');
						
						medical_clinic_get_profile_comments('post');
						
						medical_clinic_get_profile_features('details', $cmsmasters_profile_features, false, 'h3', true);
						
						medical_clinic_get_profile_category(get_the_ID(), 'pl-categs', 'post');
						
					echo '</div>';
				}
				
				
				medical_clinic_get_profile_features('features', $cmsmasters_profile_features_one, $cmsmasters_profile_features_one_title, 'h3', true);
				
				medical_clinic_get_profile_features('features', $cmsmasters_profile_features_two, $cmsmasters_profile_features_two_title, 'h3', true);
				
				medical_clinic_get_profile_features('features', $cmsmasters_profile_features_three, $cmsmasters_profile_features_three_title, 'h3', true);
				
				medical_clinic_get_profile_contact_info($cmsmasters_profile_contact_info, $cmsmasters_profile_contact_info_block_title, 'h3', 'post');
				
				medical_clinic_profile_open_social_icons($cmsmasters_profile_social, get_the_ID(), esc_html__('Socials', 'medical-clinic'), 'h3');
			echo '</div>';
		}
	echo '</div>';
	?>

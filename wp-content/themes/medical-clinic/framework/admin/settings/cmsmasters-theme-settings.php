...
add_action('admin_menu', 'medical_clinic_add_menu');
add_action('admin_init', 'medical_clinic_register_settings');
...
function medical_clinic_get_settings() {
	$output = array();
	
	
	$page = medical_clinic_get_admin_page();
	...
	case 'cmsmasters-settings':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'medical-clinic' . CMSMASTERS_THEME_STYLE;
		$cmsmasters_settings_page_title = esc_html__('Theme General Settings', 'medical-clinic');
		$cmsmasters_page_sections = medical_clinic_options_general_sections();
		$cmsmasters_page_fields = medical_clinic_options_general_fields();
		$cmsmasters_page_tabs = medical_clinic_options_general_tabs();
	...
	case 'cmsmasters-settings' . '-font':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'medical-clinic' . CMSMASTERS_THEME_STYLE . '_font';
		$cmsmasters_settings_page_title = esc_html__('Theme Fonts Settings', 'medical-clinic');
		$cmsmasters_page_sections = medical_clinic_options_font_sections();
		$cmsmasters_page_fields = medical_clinic_options_font_fields();
		$cmsmasters_page_tabs = medical_clinic_options_font_tabs();
	...
	case 'cmsmasters-settings' . '-color':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'medical-clinic' . CMSMASTERS_THEME_STYLE . '_color';
		$cmsmasters_settings_page_title = esc_html__('Theme Color Schemes', 'medical-clinic');
		$cmsmasters_page_sections = medical_clinic_options_color_sections();
		$cmsmasters_page_fields = medical_clinic_options_color_fields();
		$cmsmasters_page_tabs = medical_clinic_options_color_tabs();
	...
	case 'cmsmasters-settings' . '-element':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'medical-clinic' . CMSMASTERS_THEME_STYLE . '_element';
		$cmsmasters_settings_page_title = esc_html__('Theme Elements Settings', 'medical-clinic');
		$cmsmasters_page_sections = medical_clinic_options_element_sections();
		$cmsmasters_page_fields = medical_clinic_options_element_fields();
		$cmsmasters_page_tabs = medical_clinic_options_element_tabs();
	...
	case 'cmsmasters-settings' . '-single':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'medical-clinic' . CMSMASTERS_THEME_STYLE . '_single';
		$cmsmasters_settings_page_title = esc_html__('Theme Single Posts Settings', 'medical-clinic');
		$cmsmasters_page_sections = medical_clinic_options_single_sections();
		$cmsmasters_page_fields = medical_clinic_options_single_fields();
		$cmsmasters_page_tabs = medical_clinic_options_single_tabs();
	...
	case 'cmsmasters-settings' . '-demo':
		$cmsmasters_option_name = 'cmsmasters_options_' . 'medical-clinic' . CMSMASTERS_THEME_STYLE . '_demo';
		$cmsmasters_settings_page_title = esc_html__('Theme Settings Import/Export', 'medical-clinic');
		$cmsmasters_page_sections = medical_clinic_options_demo_sections();
		$cmsmasters_page_fields = medical_clinic_options_demo_fields();
		$cmsmasters_page_tabs = medical_clinic_options_demo_tabs();
...
function medical_clinic_create_settings_field($args = array()) {
	$defaults = array( 
		'id' => 		'default_field', 
		'title' => 		esc_html__('Default Field', 'medical-clinic'), 
		'desc' => 		esc_html__('This is a default description.', 'medical-clinic'), 
		'std' => 		'', 
		'type' => 		'text', 
...
function medical_clinic_register_settings() {
	$settings_output = medical_clinic_get_settings();
	...
	register_setting($cmsmasters_option_name, $cmsmasters_option_name, 'medical_clinic_validate_options');
	
	
	if (!empty($settings_output['cmsmasters_page_sections'])) {
		foreach ($settings_output['cmsmasters_page_sections'] as $id => $title) {
			add_settings_section($id, $title, 'medical_clinic_section_fn', __FILE__);
		}
	}
	...
	if (!empty($settings_output['cmsmasters_page_fields'])) {
		foreach ($settings_output['cmsmasters_page_fields'] as $option) {
			medical_clinic_create_settings_field($option);
		}
	}
...
function medical_clinic_settings_scripts() {
	wp_enqueue_style('wp-jquery-ui-dialog');
	
	
	wp_enqueue_style('medical-clinic-settings-css', get_template_directory_uri() . '/framework/admin/settings/css/cmsmasters-theme-settings.css', array(), '1.0.0', 'screen');
	
	
	if (is_rtl()) {
		wp_enqueue_style('medical-clinic-settings-css-rtl', get_template_directory_uri() . '/framework/admin/settings/css/cmsmasters-theme-settings-rtl.css', array(), '1.0.0', 'screen');
	}
	...
	wp_enqueue_script('medical-clinic-settings-js', get_template_directory_uri() . '/framework/admin/settings/js/cmsmasters-theme-settings.js', array('jquery', 'farbtastic'), '1.0.0', true);
	
	
	$theme_style_name = '';
	
	if (CMSMASTERS_THEME_STYLE_COMPATIBILITY) {
		foreach (medical_clinic_all_theme_styles() as $theme_style) {
	...
	wp_localize_script('medical-clinic-settings-js', 'cmsmasters_setting', array( 
		'palettes' => 						implode(',', cmsmasters_color_picker_palettes()), 
		'remove' => 						esc_attr__('Remove', 'medical-clinic'), 
		'remove_sidebar' => 				esc_html__('Do you realy want to remove this sidebar?', 'medical-clinic'), 
		'find' => 							esc_attr__('Find icons', 'medical-clinic'), 
		'remove_icon' => 					esc_html__('Do you realy want to remove this social icon?', 'medical-clinic'), 
		'theme_uri' => 						get_template_directory_uri(), 
		'shortname' => 						'medical-clinic', 
		'theme_style_name' => 				$theme_style_name, 
		'done' => 							esc_html__('All theme settings were imported successfully.', 'medical-clinic'), 
		'fail' => 							esc_html__("Error on page!!!\nPlease reload page and try again.", 'medical-clinic'), 
		'done_reset_settings' => 			esc_html__('Theme style settings were reset to default.', 'medical-clinic'), 
	...
	wp_enqueue_script('medical-clinic-settings-js-toggle', get_template_directory_uri() . '/framework/admin/settings/js/cmsmasters-theme-settings-toggle.js', array('jquery'), '1.0.0', true);
	
	wp_localize_script('medical-clinic-settings-js-toggle', 'cmsmasters_settings', array( 
		'shortname' => 	'medical-clinic' 
	));
...
function medical_clinic_add_menu() {
	$settings_output = medical_clinic_get_settings();
	...
	$add_menu_p( 
		esc_attr__('Theme Settings', 'medical-clinic'), 
		esc_attr__('Theme Settings', 'medical-clinic'), 
		'manage_options', 
		'cmsmasters-settings', 
		'medical_clinic_settings_page_fn', 
		'' 
	);
	...
	$cmsmasters_settings_general = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme General Settings', 'medical-clinic'), 
		esc_attr__('General', 'medical-clinic'), 
		'manage_options', 
		'cmsmasters-settings', 
		'medical_clinic_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_general, 'medical_clinic_settings_scripts');
	
	
	$cmsmasters_settings_font = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Fonts Settings', 'medical-clinic'), 
		esc_attr__('Fonts', 'medical-clinic'), 
		'manage_options', 
		'cmsmasters-settings' . '-font', 
		'medical_clinic_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_font, 'medical_clinic_settings_scripts');
	
	
	$cmsmasters_settings_color = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Color Schemes', 'medical-clinic'), 
		esc_attr__('Colors', 'medical-clinic'), 
		'manage_options', 
		'cmsmasters-settings' . '-color', 
		'medical_clinic_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_color, 'medical_clinic_settings_scripts');
	
	
	$cmsmasters_settings_element = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Elements Settings', 'medical-clinic'), 
		esc_attr__('Elements', 'medical-clinic'), 
		'manage_options', 
		'cmsmasters-settings' . '-element', 
		'medical_clinic_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_element, 'medical_clinic_settings_scripts');
	
	
	$cmsmasters_settings_single = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Single Posts Settings', 'medical-clinic'), 
		esc_attr__('Single Posts', 'medical-clinic'), 
		'manage_options', 
		'cmsmasters-settings' . '-single', 
		'medical_clinic_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_single, 'medical_clinic_settings_scripts');
	
	
	$cmsmasters_settings_demo = $add_submenu_p( 
		'cmsmasters-settings', 
		esc_attr__('Theme Settings Import/Export', 'medical-clinic'), 
		esc_attr__('Import/Export', 'medical-clinic'), 
		'manage_options', 
		'cmsmasters-settings' . '-demo', 
		'medical_clinic_settings_page_fn' 
	);
	
	add_action('load-' . $cmsmasters_settings_demo, 'medical_clinic_settings_scripts');
}


function medical_clinic_section_fn($desc) {
	$tab = medical_clinic_get_the_tab();
...
function medical_clinic_settings_page_fn() {
	$settings_output = medical_clinic_get_settings();
	$current_tab = medical_clinic_get_the_tab();
	
	echo '<div class="wrap">';
	
	medical_clinic_settings_page_header();
	...
	if ($current_tab != 'wpml') {
		echo '<form action="options.php" method="post" class="cmsmasters_admin_page"' . (($current_tab == 'recaptcha') ? ' style="background-color:#fdffc6; padding:35px 0 15px 25px; margin:0;"' : '') . '>' . 
			'<p class="submit-top">' . 
				(($current_tab != 'import' && $current_tab != 'export' && $current_tab != 'theme_style') ? '<input name="submit-top" type="submit" class="button button-primary button-large" value="' . esc_attr__('Save Changes', 'medical-clinic') . '" />' : '') . 
			'</p>';
			...
			echo '<p class="submit">';
				...
				if ($current_tab != 'import' && $current_tab != 'export' && $current_tab != 'theme_style') {
					echo '<input name="submit" type="submit" class="button button-primary button-large" value="' . esc_attr__('Save Changes', 'medical-clinic') . '" />';
				} elseif ($current_tab == 'import') {
					echo '<input name="import" type="button" class="button button-primary button-large cmsmasters-demo-import" value="' . esc_attr__('Import Settings', 'medical-clinic') . '" />';
				} elseif ($current_tab == 'theme_style') {
					echo '<input name="apply-theme-style" type="submit" class="button button-primary button-large cmsmasters-apply-theme-style" value="' . esc_attr__('Apply This Style', 'medical-clinic') . '" />';
					echo '<input name="reset-settings" type="button" class="button button-large cmsmasters-reset-settings" value="' . esc_attr__('Reset Theme Style Settings to Default', 'medical-clinic') . '" />';
				}
...
function medical_clinic_form_field_fn($args = array()) {
	extract($args);
	...
	case 'select_theme_style':
		$theme_style_option = get_option('cmsmasters_medical-clinic_theme_style');
		
		echo '<select id="' . $id . '" class="select' . $field_class . '" name="cmsmasters_medical-clinic_theme_style">';
	...
	case 'upload':
		...
		echo '<div class="cmsmasters_upload_parent cmsmasters_select_parent">' . 
			'<input type="button" id="cmsmasters_upload_' . $id . '_button" class="cmsmasters_upload_button button button-large" value="' . esc_attr__('Choose Image', 'medical-clinic') . '" data-title="' . esc_attr__('Choose Image', 'medical-clinic') . '" data-button="' . esc_attr__('Insert Image', 'medical-clinic') . '" data-id="cmsmasters-media-select-frame-' . $id . '" data-classes="media-frame cmsmasters-media-select-frame' . ((!isset($description)) ? ' cmsmasters-frame-no-description' : '') . ((!isset($caption)) ? ' cmsmasters-frame-no-caption' : '') . ((!isset($align)) ? ' cmsmasters-frame-no-align' : '') . ((!isset($link)) ? ' cmsmasters-frame-no-link' : '') . ((!isset($size)) ? ' cmsmasters-frame-no-size' : '') . '" data-library="image" data-type="' . $frame . '"' . (($frame == 'post') ? ' data-state="insert"' : '') . ' data-multiple="' . $multiple . '" />' . 
			'<div class="cmsmasters_upload"' . (($image != '') ? ' style="display:block;"' : '') . '>' . 
				'<img src="' . (($image != '') ? $image : '') . '" class="cmsmasters_preview_image" alt="" />' . 
				'<a href="#" class="cmsmasters_upload_cancel admin-icon-remove" title="' . esc_attr__('Remove', 'medical-clinic') . '"></a>' . 
			'</div>' . 
	...
	case 'typorgaphy':
		...
		if ($system_font) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				foreach (medical_clinic_system_fonts_list() as $key => $value) {
					echo '<option value="' . esc_attr($key) . '"' . (($options[$id . '_system_font'] == $key) ? ' selected="selected"' : '') . '>' . esc_html($value) .'</option>';
				}
				
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_system_font">' . esc_html__('System Font', 'medical-clinic') . '</label>' . 
			'</div>';
		}
		...
		if ($google_font) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_google_font">' . esc_html__('Google Font', 'medical-clinic') . '</label>' . 
			'</div>';
		}
		...
		if ($font_size) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				' &nbsp; ' . 
				'<label for="' . $id . '_font_size_number">' . esc_html__('Font Size', 'medical-clinic') . ' <em>(' . esc_html__('in pixels', 'medical-clinic') . ')</em></label>' . 
			'</div>';
		}
		...
		if ($line_height) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				' &nbsp; ' . 
				'<label for="' . $id . '_line_height_number">' . esc_html__('Line Height', 'medical-clinic') . ' <em>(' . esc_html__('in pixels', 'medical-clinic') . ')</em></label>' . 
			'</div>';
		}
		...
		if ($font_weight) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_font_weight">' . esc_html__('Font Weight', 'medical-clinic') . '</label>' . 
			'</div>';
		}
		...
		if ($font_style) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_font_style">' . esc_html__('Font Style', 'medical-clinic') . '</label>' . 
			'</div>';
		}
		...
		if ($text_transform) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_text_transform">' . esc_html__('Text Transform', 'medical-clinic') . '</label>' . 
			'</div>';
		}
		...
		if ($text_decoration) {
			echo '<div class="cmsmasters_admin_block">' . 
				...
				foreach (medical_clinic_text_decoration_list() as $key => $value) {
					echo '<option value="' . esc_attr($key) . '"' . (($options[$id . '_text_decoration'] == $key) ? ' selected="selected"' : '') . '>' . esc_html($value) .'</option>';
				}
				
				echo '</select>' . 
				' &nbsp; ' . 
				'<label for="' . $id . '_text_decoration">' . esc_html__('Text Decoration', 'medical-clinic') . '</label>' . 
			'</div>';
		}
	...
	case 'sidebar':
		...
		'<div class="sidebar_management">' . 
			'<p>' . 
				...
				'<input class="button" type="button" id="add_sidebar" value="' . esc_attr__('Add Sidebar', 'medical-clinic') . '" />' . 
			'</p>' . 
			'<div></div>' . 
			'<ul>';
	...
	case 'social':
		...
		'<div class="icon_management">' . 
			'<p>' . 
				...
				'<input id="' . $id . '_button" class="cmsmasters_icon_choose_button button" type="button" value="' . esc_attr__('Choose icon', 'medical-clinic') . '" />' . 
				'<a href="#" class="cmsmasters_remove_icon admin-icon-remove" title="' . esc_attr__('Cancel changes', 'medical-clinic') . '"></a>' . 
			'</p>' . 
			'<span class="cl"><br /></span>' . 
			'<span class="icon_upload_link" style="display:none;">' . 
				'<label for="new_icon_color">' . 
					'<span class="cmsmasters_col_label">' . esc_html__('Icon Color', 'medical-clinic') . '</span>' . 
				...
				'<label for="new_icon_hover">' . 
					'<span class="cmsmasters_col_label">' . esc_html__('Icon Hover Color', 'medical-clinic') . '<br /><em>' . esc_html__('if not selected - Icon Color will be used', 'medical-clinic') . '</em></span>' . 
				...
				'<label for="new_icon_link">' . 
					...
					esc_html__('Icon Link', 'medical-clinic') . 
				'</label>' . 
				'<label for="new_icon_title">' . 
					'<input class="all-options" type="text" id="new_icon_title" /> ' . 
					esc_html__('Icon Title', 'medical-clinic') . 
				'</label>' . 
				'<label for="new_icon_target">' . 
					'<input type="checkbox" id="new_icon_target" value="true" /> ' . 
					esc_html__('Open link in a new tab/window?', 'medical-clinic') . 
				'</label>' . 
			'</span>' . 
			'<span class="cl"></span>' . 
			'<input class="button button-primary" type="button" id="add_icon" value="' . esc_attr__('Add Icon', 'medical-clinic') . '" />' . 
			'<input class="button button-primary" type="button" id="edit_icon" value="' . esc_attr__('Save Icon', 'medical-clinic') . '" />' . 
			'<ul>';
			...
			if (isset($options[$id]) && is_array($options[$id])) {
				foreach($options[$id] as $icon) {
					...
					echo '<li>' . 
						...
						'</div>' . 
						'<a href="#" class="icon_del admin-icon-remove" title="' . esc_attr__('Remove', 'medical-clinic') . '"></a> ' . 
...
function medical_clinic_validate_options($input) {
	$valid_input = array();
	...
	foreach ($options as $option) {
		...
		case 'text':
			...
			case 'numeric':
				...
				$valid_input[$option['id']] = (is_numeric($input[$option['id']])) ? $input[$option['id']] : esc_html__('Expecting a Numeric value!', 'medical-clinic');
				
				if (is_numeric($input[$option['id']]) == false) {
					add_settings_error(
						$option['id'],
						'medical-clinic' . '_txt_numeric_error',
						esc_html__('Expecting a Numeric value! Please fix.', 'medical-clinic'),
						'error'
					);
				}
			...
			case 'multinumeric':
				...
				if ($input[$option['id']] != '') {
					$valid_input[$option['id']] = (preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) == 1) ? $input[$option['id']] : esc_html__('Expecting comma separated numeric values', 'medical-clinic');
				} else {
				...
				if ($input[$option['id']] != '' && preg_match('/^-?\d+(?:,\s?-?\d+)*$/', $input[$option['id']]) != 1) {
					add_settings_error(
						$option['id'],
						'medical-clinic' . '_txt_multinumeric_error',
						esc_html__('Expecting comma separated numeric values! Please fix.','medical-clinic'),
						'error'
					);
				}
			...
			case 'email':
				...
				if ($input[$option['id']] != '') {
					$valid_input[$option['id']] = (is_email($input[$option['id']]) !== false) ? $input[$option['id']] : esc_html__('Invalid email! Please re-enter!', 'medical-clinic');
				} elseif ($input[$option['id']] == '') {
					$valid_input[$option['id']] = esc_html__('This setting field cannot be empty! Please enter a valid email address.', 'medical-clinic');
				}
				
				if (is_email($input[$option['id']]) == false || $input[$option['id']] == '') {
					add_settings_error(
						$option['id'],
						'medical-clinic' . '_txt_email_error',
						esc_html__('Please enter a valid email address.', 'medical-clinic'),
						'error'
					);
				}
		...
		case 'number':
			$input[$option['id']] = trim($input[$option['id']]);
			$valid_input[$option['id']] = (is_numeric($input[$option['id']])) ? $input[$option['id']] : esc_html__('Number!', 'medical-clinic');
...
function medical_clinic_show_msg($message, $msgclass = 'info') {
	echo '<div id="message" class="' . $msgclass . '">' . $message . '</div>';
}


function medical_clinic_admin_msgs() {
	global $pagenow;
...
function medical_clinic_add_global_options($reset = false) {
	...
	foreach (medical_clinic_all_color_schemes_list() as $key => $value) {
		array_push($cmsmasters_option_names, array( 
			'cmsmasters_options_' . 'medical-clinic' . CMSMASTERS_THEME_STYLE . '_color_' . $key, 
			medical_clinic_options_color_fields($key) 
		));
	}
		...
		if ($cmsmasters_option_name[1] !== '') {
			foreach ($cmsmasters_option_name[1] as $selected_option) {
				if ( 
					...
					$selected_option['id'] !== 'medical-clinic' . '_social_icons' 
				) {
...
function medical_clinic_get_global_options() {
	...
	foreach (medical_clinic_all_color_schemes_list() as $key => $value) {
		array_push($cmsmasters_option_names, 'cmsmasters_options_' . 'medical-clinic' . CMSMASTERS_THEME_STYLE . '_color_' . $key);
	}

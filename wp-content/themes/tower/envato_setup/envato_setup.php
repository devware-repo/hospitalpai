...
if ( ! class_exists( 'Envato_Theme_Setup_Wizard' ) ) {
	...
	class Envato_Theme_Setup_Wizard {
		...
		public function admin_menus() {
			...
			if( $this->is_submenu_page() ){
				...
				$add_subpage_function( $this->parent_slug, __( 'Setup Wizard','codeless' ), __( 'Setup Wizard','codeless' ), 'manage_options', $this->page_slug, array( $this, 'setup_wizard' ) );
			}else{
				add_theme_page( __( 'Setup Wizard','codeless' ), __( 'Setup Wizard','codeless' ), 'manage_options', $this->page_slug, array( $this, 'setup_wizard' ) );
			}
		...
		public function setup_wizard() {
			...
			wp_localize_script( 'envato-setup', 'envato_setup_params', array(
				...
				'verify_text' => __( '...verifying','codeless' ),
			) );
	...
	public function setup_wizard_header() {
		...
		<?php if ( 'next_steps' === $this->step ) : ?>
			<a class="wc-return-to-dashboard" href="<?php echo esc_url( admin_url() ); ?>"><?php _e( 'Return to the WordPress Dashboard', 'codeless' ); ?></a>
		<?php endif; ?>
		</body>
		<?php
...
//				exit;
				...
				?>
				<h1><?php printf( __( 'Welcome to the setup wizard for %s.', 'codeless' ), wp_get_theme()); ?></h1>
				<p><?php _e('It looks like you may have recently upgraded to this theme. Great! This setup wizard will help ensure all the default settings are correct. It will also show some information about your new website and support options.', 'codeless');?></p>
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
					   class="button-primary button button-large button-next"><?php _e( 'Let\'s Go!', 'codeless' ); ?></a>
					<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(),'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
					   class="button button-large"><?php _e( 'Not right now', 'codeless' ); ?></a>
				</p>
				<?php
			} else if( get_option('envato_setup_complete', false) ){
				?>
				<h1><?php printf( __( 'Welcome to the setup wizard for %s.', 'codeless' ), wp_get_theme()); ?></h1>
				<p><?php _e('It looks like you have already run the setup wizard. Below are some options: ', 'codeless');?></p>
				<ul>
					<li>
						<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>"
						   class="button-primary button button-next button-large"><?php _e( 'Run Setup Wizard Again', 'codeless' ); ?></a>
					</li>
					<li>
						<form method="post">
							<input type="hidden" name="reset-font-defaults" value="yes">
							<input type="submit" class="button-primary button button-large button-next" value="<?php esc_attr_e( 'Reset font style and colors', 'codeless' ); ?>" name="save_step" />
				...
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( wp_get_referer() && ! strpos( wp_get_referer(),'update.php' ) ? wp_get_referer() : admin_url( '' ) ); ?>"
					   class="button button-large"><?php _e( 'Cancel', 'codeless' ); ?></a>
				</p>
				<?php
			} else {
				?>
				<h1><?php printf( __( 'Welcome to the setup wizard for %s.', 'codeless' ), wp_get_theme()); ?></h1>
				<p><?php printf( __( 'Thank you for choosing the %s theme from ThemeForest. This quick setup wizard will help you configure your new website. This wizard will install the required WordPress plugins, default content, logo and tell you a little about Help &amp; Support options. <br/>It should only take 5 minutes.' , 'codeless' ), wp_get_theme()); ?></p>
				<p><?php _e( 'No time right now? If you don\'t want to go through the wizard, you can skip and return to the WordPress dashboard. Come back anytime if you change your mind!', 'codeless' ); ?></p>

				<h2><?php printf( __( 'Warning ! Read Me', 'codeless' ), wp_get_theme()); ?></h2>
				<p> If you will experince certain issue during installation caused by hosting provider please follow these steps:</p>
				<ul>
					<li>1. Install the Plugins with Wizard and skip the content installation </li>
					<li>2. Go to WP Dashboard > Tower > Import/Export   </li>
					<li>3. Install the dummy data without images attachments.   </li>
					<li>4. Everything will be installed without issue.  </li>
		...
		public function envato_setup_introduction_save(){
			...
			if(!empty($_POST['reset-font-defaults']) && $_POST['reset-font-defaults'] == 'yes'){
				...
				update_option('cl_redata', $data);

				$data1 = ReduxFrameworkInstances::get_instance('cl_redata')->options_defaults;

				update_option('cl_redata', $data1);
		...
		public function envato_setup_default_plugins() {
			...
			?>
			<h1><?php _e( 'Default Plugins', 'codeless' ); ?></h1>
			<form method="post">
				...
				if ( count( $plugins['all'] ) ) {
					?>
					<p><?php _e( 'Your website needs a few essential plugins. The following plugins will be installed or updated:', 'codeless' ); ?></p>
					...
					echo '<p><strong>'._e( 'Good news! All plugins are already installed and up to date. Please continue.','codeless' ).'</strong></p>';
				} ?>

				<p><?php _e( 'You can add and remove plugins later on from within WordPress.', 'codeless' ); ?></p>

				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button-primary button button-large button-next" data-callback="install_plugins"><?php _e( 'Continue', 'codeless' ); ?></a>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next"><?php _e( 'Skip this step', 'codeless' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
			</form>
			<?php
		...
		public function ajax_plugins() {
			if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['slug'] ) ) {
				wp_send_json_error( array( 'error' => 1, 'message' => __( 'No Slug Found','codeless' ) ) );
			}
			$json = array();
			...
			foreach ( $plugins['activate'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2' => -1,
						'message' => __( 'Activating Plugin','codeless' ),
					);
					break;
				}
			}
			foreach ( $plugins['update'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2' => -1,
						'message' => __( 'Updating Plugin','codeless' ),
					);
					break;
				}
			}
			foreach ( $plugins['install'] as $slug => $plugin ) {
				if ( $_POST['slug'] == $slug ) {
					$json = array(
						...
						'action2' => -1,
						'message' => __( 'Installing Plugin','codeless' ),
					);
					break;
				}
			}
			...
			if ( $json ) {
				...
				wp_send_json( array( 'done' => 1, 'message' => __( 'Success','codeless' ) ) );
			}
			exit;
		...
		private function _content_default_get() {
			...
			$available_content = $this->_get_json( 'default.json', $dir);
			foreach($available_content as $post_type => $post_data){
				if(count($post_data)){
					...
					$content[$post_type] = array(
						...
						'description' => sprintf( __( 'This will create default %s as seen in the demo.', 'codeless' ), $post_type_title ),
						'pending' => __( 'Pending.', 'codeless' ),
						'installing' => __( 'Installing.', 'codeless' ),
						'success' => __( 'Success.', 'codeless' ),
			...
			$content['widgets'] = array(
				'title' => __( 'Widgets', 'codeless' ),
				'description' => __( 'Insert default sidebar widgets as seen in the demo.', 'codeless' ),
				'pending' => __( 'Pending.', 'codeless' ),
				'installing' => __( 'Installing Default Widgets.', 'codeless' ),
				'success' => __( 'Success.', 'codeless' ),
			...
			$content['revslider'] = array(
				'title' => __( 'Revolution Slider', 'codeless' ),
				'description' => __( 'Install Revolution Sliders.', 'codeless' ),
				'pending' => __( 'Pending.', 'codeless' ),
				'installing' => __( 'Installing Revolution Sliders... ', 'codeless' ),
				'success' => __( 'Success.', 'codeless' ),
				'install_callback' => array( $this,'_content_install_revsliders' ),
				'checked' => $this->is_possible_upgrade() ? 0 : 1 // dont check if already have content installed.
			);

			$content['options'] = array(
				'title' => __( 'Theme Option', 'codeless' ),
				'description' => __( 'Configure default settings.', 'codeless' ),
				'pending' => __( 'Pending.', 'codeless' ),
				'installing' => __( 'Installing Default Settings.', 'codeless' ),
				'success' => __( 'Success.', 'codeless' ),
				'install_callback' => array( $this,'_content_install_options' ),
				'checked' => $this->is_possible_upgrade() ? 0 : 1 // dont check if already have content installed.
			);
			...
			$content['attachments'] = array(
				'title' => __( 'Install Images', 'codeless' ),
				'description' => __( 'Install Img and attachments', 'codeless' ),
				'pending' => __( 'Pending.', 'codeless' ),
				'installing' => __( 'Installing attachments.', 'codeless' ),
				'success' => __( 'Success.', 'codeless' ),
				'install_callback' => array( $this,'_content_install_attachments' ),
				'checked' => $this->is_possible_upgrade() ? 0 : 1 // dont check if already have content installed.
			);

           
		    $content['demo'] = array(
				'title' => __( 'Install Content', 'codeless' ),
				'description' => __( 'Install Content of the Demo.', 'codeless' ),
				'pending' => __( 'Pending.', 'codeless' ),
				'installing' => __( 'Installing Demo.', 'codeless' ),
				'success' => __( 'Success.', 'codeless' ),
				'demos' => $this->listdemos(),
				'install_callback' => array( $this,'_content_install_demo' ),
				//'checked' => $this->is_possible_upgrade() ? 0 : 1 // dont check if already have content installed.
			);

			$content['menu'] = array(
				'title' => __( 'Menu', 'codeless' ),
				'description' => __( 'Insert default menu as seen in the demo.', 'codeless' ),
				'pending' => __( 'Pending.', 'codeless' ),
				'installing' => __( 'Installing Default Menu.', 'codeless' ),
				'success' => __( 'Success.', 'codeless' ),
		...
		public function listdemos(){
			...
			$array = array();	
			$path = get_template_directory() . '/includes/dummy_data/demos';
			$path_img = get_template_directory_uri() . '/includes/dummy_data/img/';
			$scandir = scandir($path);
			$folders = array_diff($scandir, array('.', '..', '.DS_Store'));

				foreach ($folders as $folder) :

					$array[$folder]['name'] = $folder;
				    $array[$folder]['img'] = $path_img.$folder.'.jpg';
		...
		public function envato_setup_default_content() {
			?>
			<h1><?php _e( 'Choose one demo to be installed', 'codeless' ); ?></h1>
			<form method="post">
				<?php if($this->is_possible_upgrade()){ ?>
					<p><?php _e('It looks like you already have content installed on this website. If you would like to install the default demo content as well you can select it below. Otherwise just choose the upgrade option to ensure everything is up to date.', 'codeless'); ?></p>
				<?php }else{ ?>
					<p><?php printf( __( 'It\'s time to insert some default content for your new WordPress website. Choose what you would like inserted below and click Continue. It is recommended to leave everything selected. Once inserted, this content can be managed from the WordPress admin dashboard. ', 'codeless' ), '<a href="' . esc_url( admin_url( 'edit.php?post_type=page' ) ) . '" target="_blank">', '</a>' ); ?></p>
				<?php } ?>
				...
				<table class="envato-setup-pages" cellspacing="0">
					...
					<tr>
						<td class="check"> </td>
						<th class="item"><?php _e( 'Item', 'codeless' ); ?></th>
						<th class="description"><?php _e( 'Description', 'codeless' ); ?></th>
						<th class="status"><?php _e( 'Status', 'codeless' ); ?></th>
					</tr>
					</thead>
					...
					<tbody>
					<input type="hidden" name="demo" id="demo" value="default" />
					<?php foreach ( $this->_content_default_get() as $slug => $default ) {  
						if(isset($default['demos'])): 
						
							foreach($default['demos'] as $demo): ?>
								
								<div class="demo_screenshot" data-content="<?php echo esc_attr($demo['name']) ?>"><img width=195px" height="136px" src="<?php echo esc_attr($demo['img']) ?>"/> <p><?php echo esc_attr($demo['name']) ?></p></div>
						...
						<tr class="envato_default_content" data-content="<?php echo esc_attr( $slug );?>">
							...
							<td><label for="default_content_<?php echo esc_attr( $slug );?>"><?php echo esc_attr($default['title']); ?></label></td>
							<td class="description"><?php echo esc_attr($default['description']); ?></td>
							<td class="status"> <span><?php echo esc_attr($default['pending']);?></span> <div class="spinner"></div></td>
						</tr>
					<?php } ?>
					</tbody>
				</table>
				...
				<p class="envato-setup-actions step">
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button-primary button button-large button-next" data-callback="install_content"><?php _e( 'Continue', 'codeless' ); ?></a>
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next"><?php _e( 'Skip this step', 'codeless' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
			</form>
		...
		public function ajax_content() {
			...
			if ( ! check_ajax_referer( 'envato_setup_nonce', 'wpnonce' ) || empty( $_POST['content'] ) && isset( $content[ $_POST['content'] ] ) ) {
				wp_send_json_error( array( 'error' => 1, 'message' => __( 'No content Found','codeless' ) ) );
			}
			...
			if ( $json ) {
				...
				wp_send_json( array( 'error' => 1, 'message' => __( 'Error','codeless' ) ) );
			}
...
//									echo "Post meta $meta_key was $meta_val \n\n";
		...
		private function _content_install_type(){
			...
			$all_data = $this->_get_json('default.json', $dir);
		...
		private function _fetch_remote_file( $url, $post ) {
			...
			if ( !$upload || $upload['error'] ) {
				...
				if ( is_array( $response ) && !empty($response['body']) && $response['response']['code'] == '200' ) {
					...
					@unlink( $upload['file'] );
					return new WP_Error( 'import_file_error', __( 'Remote server did not respond', 'codeless' ) );
				}
				...
				if ( isset( $headers['content-length'] ) && $filesize != $headers['content-length'] ) {
					...
					return new WP_Error( 'import_file_error', __( 'Remote file is incorrect size', 'codeless' ) );
				}

				if ( 0 == $filesize ) {
					@unlink( $upload['file'] );

					return new WP_Error( 'import_file_error', __( 'Zero size file downloaded', 'codeless' ) );
				}

				if ( ! empty( $max_size ) && $filesize > $max_size ) {
					@unlink( $upload['file'] );

					return new WP_Error( 'import_file_error', sprintf( __( 'Remote file is too large, limit is %s', 'codeless' ), size_format( $max_size ) ) );
				}
			}
		...
		private function _content_install_widgets() {
			...
			if($options){
				...
				$msg = '<br />'.__("Widgets Imported !", "codeless");
				return true;
			}else{
				$msg = '<br />'.__("Your backup or demo doesn't contain sidebar_widgets.txt file.", "codeless");
				return false;
			}
	    ...
	    public function prepareFile(){
	    	$demo = $_POST['demo'];
	    	$zip = 'default';
	    	$path = 'http://codeless.co/tower_dummies/'.$zip.'.zip';
			$new_file = get_template_directory().'/downloads/'.$demo.'.zip';

			if(!file_exists(get_template_directory().'/downloads/'))
				wp_mkdir_p(get_template_directory().'/downloads/');
			
			if($this->downloadFile($path, $new_file)){

				// get the absolute path to $file
				$path_full = pathinfo(realpath($new_file), PATHINFO_DIRNAME);

				$zip = new ZipArchive;
				$res = $zip->open($new_file);
				if ($res === TRUE) {
				  // extract it to the path we determined above
				  $zip->extractTo($path_full);
				  $zip->close();
	    ...
	    public function modifyXml (){
	    	$dir = $_POST['demo'];
	    	$file = 'content_1';
		    $path = get_template_directory() . '/includes/dummy_data/demos/'.$dir.'/' . $file.'.xml';
				// load the document
				...
				foreach($items as $item){
					...
					$wp_children->attachment_url = str_replace('http://codeless.co/tower/default/wp-content/uploads/', get_template_directory_uri().'/downloads/uploads/', $wp_children->attachment_url);

				}

				// save the updated document
				if($channel->asXML(get_template_directory() . '/includes/dummy_data/demos/'.$dir.'/'.$file.'.xml'))
					return true;
		...
		public function _content_install_menu(){
			...
			if($menus_data){
				...
				$this->message .= '<br />'.__("Menu Imported successfully", "codeless");
				return true;
			}else{
				$this->message .= '<br />'.__("Your backup or demo doesn't contain sidebar_widgets.txt file.", "codeless");
				return true;
			}
		...
		public function _content_install_revsliders(){
		   ...
		   $dir = $_POST['demo'];	
			...
			$slider_array = array('slider' => 'slider_zips/'.$dir.'/slider.zip', 'slider2'=>'slider_zips/business2/business2_about.zip' );

			
			foreach ($slider_array as $sliders => $slider_url  ) {
				

					$uri = 'http://codeless.co/tower/'.$slider_url;
		...
		//import options
		public function _content_install_options(){
			$dir = $_POST['demo'];

			$options = $this->read_file('options.txt', $dir);
			if($options){
				update_option('cl_redata', $options);
				
				$msg = '<br />'.__("Options imported successfully", "codeless");
				return true;
			}else{
				$msg = '<br />'.__("Your backup doesn't contain options.txt file.", "codeless");
				return false;
			}
		}

		public function _content_install_attachments(){
	    ...
	    public function _content_install_demo() {

	    	$dir = $_POST['demo'];
	    	$attachments = $_POST['attachments'];
	    	$zip = $_POST['demo'].'new';

	    	$file =array('data' => 'content_1');

	    	if($dir == 'default')
	    		$file = array('data' => 'content_1', 'data1' => 'content_2');
				...
				require_once( ABSPATH. 'wp-content/themes/tower/admin/inc/fields/codeless_import/class.wordpress-importer.php');
				...
				foreach($file as $file=>$file_url):
			
			     	$path = get_template_directory() . '/includes/dummy_data/demos/'.$dir.'/'. $file_url.'.xml';
				...
				if(is_wp_error($value)){
					$this->message .= __("Error during Import. Try Again", "codeless");
					
					return true;
				}
				else {
					$this->message .= __("Content has been imported successfully ", "codeless");
		...
		private function _get_json( $file, $dir ) {
			if ( is_file( __DIR__.'/content/'.$dir.'/'.basename( $file ) ) ) {
				WP_Filesystem();
				global $wp_filesystem;
				$file_name = __DIR__ .'/content/'.$dir.'/' . basename( $file );
				$file = base64_decode($file_name);
		...
		public function read_file($file, $dir){
			...
			$file = get_template_directory() . '/includes/dummy_data/demos/'.$dir.'/'. $file;
			
			if ( file_exists($file) ) {
				$content = $this->get_content($file);
			} else {
				$this->message = __("File doesn't exist", "codeless");
			}
		...
		public function envato_setup_logo_design() {
			 global $cl_redata;
			?>
			<h1><?php _e( 'Logo &amp; Design', 'codeless' ); ?></h1>
			<form method="post">
				<p><?php printf( __( 'Please add your logo below. For best results, the logo should be a transparent PNG ( 466 by 277 pixels). The logo can be changed at any time from the Appearance > Customize area in your dashboard.', 'codeless'), ''); ?></p>

				<table>
					<tr>
						<td>
							<div id="current-logo">
								<?php $image_url = $cl_redata['logo']['url'];
							    

								//$image_url = apply_filters('envato_setup_logo_image',$image_url);
								if ( $image_url ) {
									$image = '<img class="site-logo" src="%s" alt="%s" style="width:auto; height:auto" />';
									printf(
										$image,
										$image_url,
										get_bloginfo( 'name' ),
						...
						<td>
							<a href="#" class="button button-upload"><?php _e( 'Upload New Logo', 'codeless' ); ?></a>
						</td>
					</tr>
				</table>
				...
				if( !$this->get_default_theme_style() || !$demo_styles){
					...
					<p><?php _e( 'Please choose the color scheme for this website. The color scheme (along with font colors &amp; styles) can be changed at any time from the Appearance > Customize area in your dashboard.' ,'codeless' ); ?></p>
					...
					<div class="theme-presets">
						<ul>
							...
							foreach ( $demo_styles as $demo_name => $demo_style ) {
								?>
								<li<?php return $demo_name == $current_demo ? ' class="current" ' : '';?>>
				...
				<p><em>Please Note: Advanced changes to website graphics/colors may require extensive PhotoShop and Web Development knowledge. We recommend hiring an expert from <a href="<?php echo esc_url("http://studiotracking.envato.com/aff_c?offer_id=4&aff_id=1564&source=DemoInstall")?>" target="_blank">Envato Studio</a> to assist with any advanced website changes.</em></p>
				...
				<p class="envato-setup-actions step">
					<input type="submit" class="button-primary button button-large button-next" value="<?php esc_attr_e( 'Continue', 'codeless' ); ?>" name="save_step" />
					<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next"><?php _e( 'Skip this step', 'codeless' ); ?></a>
					<?php wp_nonce_field( 'envato-setup' ); ?>
				</p>
			</form>
		...
		public function envato_setup_logo_design_save() {
			...
			if ( $new_logo_id ) {
				...
				if ( $attr && ! empty( $attr[1] ) && ! empty( $attr[2] ) ) {
					$cl_redata['logo']['url'] = $attr[0];
					update_option('cl_redata', $cl_redata);
		...
		public function envato_setup_updates() {
			?>
			<h1><?php _e( 'Theme Updates', 'codeless' ); ?></h1>
			<?php if ( function_exists( 'envato_market' ) ) { ?>
				<form method="post">
					...
					if ( count( $my_items ) ) {
						...
						<p class="envato-setup-actions step">
							<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next button-primary"><?php _e( 'Continue', 'codeless' ); ?></a>
						</p>
						<?php
					} else {
						?>
						<p><?php _e( 'Please login using your ThemeForest account to enable Theme Updates. We update themes when a new feature is added or a bug is fixed. It is highly recommended to enable Theme Updates.', 'codeless' ); ?></p>
						<p>When an update becomes available it will show in the Dashboard with an option to install.</p>
						<p>
							<em>On the next page you will be asked to Login with your ThemeForest account and grant permissions to enable Automatic Updates. If you have any questions please <a href="<?php echo esc_url("http://codeless.co")?>" target="_blank">contact us</a>.</em>
						</p>
						<p class="envato-setup-actions step">
							<input type="submit" class="button-primary button button-large button-next" value="<?php esc_attr_e( 'Login with Envato', 'codeless' ); ?>" name="save_step" />
							<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-large button-next"><?php _e( 'Skip this step', 'codeless' ); ?></a>
							<?php wp_nonce_field( 'envato-setup' ); ?>
						</p>
					<?php } ?>
		...
		public function envato_setup_customize() {
			...
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-primary button-large button-next"><?php _e( 'Continue', 'codeless' ); ?></a>
			</p>
		...
		public function envato_setup_help_support() {
			...
			<p>Item Support can be accessed from <a href="<?php echo esc_url("http://codeless.co/support")?>" target="_blank">http://codeless.co/support</a> and includes:</p>
			<ul>
			...
			<ul>
				...
				<li>Installation services & Hosting (this is available through <a href="<?php echo esc_url("http://Beemple.com") ?>" target="_blank">Beemple.com</a>)</li>
				<li>Help and Support for non-bundled 3rd party plugins (i.e. plugins you install yourself later on)</li>
			</ul>
			<p>More details about item support can be found in the ThemeForest <a href="<?php echo esc_url("http://themeforest.net/page/item_support_policy") ?>" target="_blank">Item Support Polity</a>. </p>
			<p class="envato-setup-actions step">
				<a href="<?php echo esc_url( $this->get_next_step_link() ); ?>" class="button button-primary button-large button-next"><?php _e( 'Agree and Continue', 'codeless' ); ?></a>
				<?php wp_nonce_field( 'envato-setup' ); ?>
			</p>
			<?php
		}
		...
		public function envato_setup_ready() {
			...
			?>
			<a href="<?php echo esc_url("https://twitter.com/share") ?>" class="twitter-share-button" data-url="http://themeforest.net/user/code-less/portfolio?ref=code-less" data-text="<?php echo esc_attr( 'I just installed the ' . wp_get_theme() .' #WordPress theme from #ThemeForest' ); ?>" data-via="EnvatoMarket" data-size="large">Tweet</a>
			<script>!function(d,s,id){var js,fjs=d.getElementsByTagName(s)[0];if(!d.getElementById(id)){js=d.createElement(s);js.id=id;js.src="//platform.twitter.com/widgets.js";fjs.parentNode.insertBefore(js,fjs);}}(document,"script","twitter-wjs");</script>

			<h1><?php _e( 'Your Website is Ready!', 'codeless' ); ?></h1>
			...
			<p>Please come back and <a href="<?php echo esc_url("http://themeforest.net/downloads") ?> " target="_blank">leave a 5-star rating</a> if you are happy with this theme. <br/>Follow <a  href="https://twitter.com/codelessthemes" target="_blank">@codelessthemes</a> on Twitter to see updates. Thanks! </p>

			<div class="envato-setup-next-steps">
				<div class="envato-setup-next-steps-first">
					<h2><?php _e( 'Next Steps', 'codeless' ); ?></h2>
					<ul>
						<li class="setup-product"><a class="button button-primary button-large" href="https://twitter.com/codelessthemes" target="_blank"><?php _e( 'Follow @codelessthemes on Twitter', 'codeless' ); ?></a></li>
						<li class="setup-product"><a class="button button-next button-large" href="<?php echo esc_url( home_url('/') ); ?>"><?php _e( 'View your new website!', 'codeless' ); ?></a></li>
					</ul>
				</div>
				<div class="envato-setup-next-steps-last">
					<h2><?php _e( 'More Resources', 'codeless' ); ?></h2>
					<ul>
						<li class="documentation"><a href="<?php echo esc_url("http://codeless.co/documentation")?>" target="_blank"><?php _e( 'Read the Theme Documentation', 'codeless' ); ?></a></li>
						<li class="howto"><a href="https://wordpress.org/support/" target="_blank"><?php _e( 'Learn how to use WordPress', 'codeless' ); ?></a></li>
						<li class="rating"><a href="<?php echo esc_url("http://themeforest.net/downloads") ?>" target="_blank"><?php _e( 'Leave an Item Rating', 'codeless' ); ?></a></li>
						<li class="support"><a href="<?php echo esc_url("http://codeless.co")?>" target="_blank"><?php _e( 'Get Help and Support', 'codeless' ); ?></a></li>
					</ul>
				</div>
			</div>
			<?php
		...
		public function envato_market_admin_init() {
			...
			if ( ! empty( $_POST['oauth_session'] ) && ! empty( $_POST['bounce_nonce'] ) && wp_verify_nonce( $_POST['bounce_nonce'], 'envato_oauth_bounce_' . $this->envato_username ) ) {
				...
				if ( is_wp_error( $response ) ) {
					...
					$class = 'error';
					echo "<div class=\"$class\"><p>".sprintf( __( 'Something went wrong while trying to retrieve oauth token: %s','codeless' ), $error_message ).'</p></div>';
				} else {
			...
			add_settings_section(
				...
				sprintf( __( 'Login for %s updates', 'codeless' ), $this->envato_username ),

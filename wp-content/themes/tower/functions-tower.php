...
function codeless_author_box(){
  $output = '<dl class="author_box dl-horizontal">';
    $output .= '<dt>'.get_avatar(esc_attr(get_the_author_meta( 'ID' ) ), 64).'</dt>';
    $output .= '<dd>';
      $output .= '<h5>'.__('About', 'codeless').' '.esc_attr(get_the_author()).'</h5>';
      $output .= '<p>'.esc_attr(get_the_author_meta('description')).'</p>';
    $output .= '</dd>';
  $output .= '</dl>';
...
function codeless_pagination($pages = '', $range = 2){
    ...
    $showitems = ($range * 2)+1;  
     ...
     if(1 != $pages)
     {
         echo "<div class=\"pagination\"><span>Page ".esc_attr($paged)." of ".esc_attr($pages)."</span>";
         ...
         for ($i=1; $i <= $pages; $i++)
         {
             ...
             {  
                 ...
                 if($paged + 1 == $i)
                  $next_link = 'next_link';
                 echo ($paged == $i)? "<span class=\"current\">".$i."</span>":"<a href='".esc_url(get_pagenum_link($i))."' class=\"inactive ".$next_link." \">".$i."</a>";
             }
         }
...
function codeless_pagination_display(){
  ...
  echo '<div class="p_pagination '.esc_attr($show).' '.esc_attr($infinite).'">';
  if( ($cl_current_view == 'blog' && $cl_redata['blog_pagination'] == 'load_more' ) || ($cl_current_view == 'portfolio' && $cl_redata['portfolio_pagination'] == 'load_more') )
    echo '<a href="#" class="btn-bt '.esc_attr($cl_redata['overall_button_style'][0]).' load_more_button">'.__('Load More', 'codeless').'</a>';
  
  if($cl_current_view == 'portfolio' && $cl_redata['portfolio_pagination'] == 'no_pagination'){
    echo '</div>';
    return;
  }
...
function codeless_register_scripts(){
    ...
    wp_register_script('snap', CODELESS_BASE_URL.'js/snap.js', array('jquery'), 1, true );
    ...
    wp_register_script('owl.carousel.min', CODELESS_BASE_URL.'js/owl.carousel.min.js', array('jquery'), 1, true );
    wp_register_script('jquery.animsition.min', CODELESS_BASE_URL.'js/jquery.animsition.min.js', array('jquery'), 1, true );
} 
if(!is_admin()){
...
function codeless_register_styles(){   
    ...
    wp_register_style( 'owl.carousel',CODELESS_BASE_URL.'css/owl.carousel.css' ); 
    wp_register_style( 'owl.theme',CODELESS_BASE_URL.'css/owl.theme.css' ); 
    wp_register_style( 'animsition',CODELESS_BASE_URL.'css/animsition.min.css' );
}
if(!is_admin()){
...
add_filter('body_class', 'codeless_outter_padding');
function codeless_outter_padding($classes = ''){
    global $cl_redata;

    if(isset($cl_redata['outter_padding']) && $cl_redata['outter_padding'])
...
function codeless_fullwidthheader($classes = ''){
    ...
    if(isset($cl_redata['header_container_full']) && $cl_redata['header_container_full'] && $cl_redata['header_style'] != 'header_7') 
      $classes[] = 'fullwidth_header';
    return $classes;
}

add_filter('body_class', 'codeless_header_tools_divider');
function codeless_header_tools_divider($classes = ''){
    global $cl_redata;

    if(isset($cl_redata['header_tools_divider']) && $cl_redata['header_tools_divider'] && $cl_redata['header_style'] != 'header_7')
      $classes[] = 'header_tools_divider';
    return $classes;
}

add_filter('body_class', 'codeless_header_transparent');
function codeless_header_transparent($classes = ''){
    global $cl_redata;
    
    if(isset($cl_redata['header_transparency']) && $cl_redata['header_transparency'])
...
function codeless_header_transitions($type){
  global $cl_redata;
  
  $transition['class'] = '';
  $transition['attr'] = '';

  if($cl_redata['codeless_page_transition']):
    $transition_attr['attr'] = 'data-animsition-in="'.esc_attr($cl_redata['page_transition_in']).'" data-animsition-in-duration="'.esc_attr($cl_redata['page_transition_in_duration']).'" data-animsition-out="'.esc_attr($cl_redata['page_transition_out']).'" data-animsition-out-duration="'.esc_attr($cl_redata['page_transition_out_duration']).'"';
...
function codeless_header_bgCheck(){
  global $cl_redata;

  $bgCheck = '';

  if($cl_redata['header_transparency']){
      if((int) codeless_get_post_id() != 0)
          $page_header_menu_color = redux_post_meta('cl_redata',(int) codeless_get_post_id(), 'page_header_menu_color');
      ...
      $p_bool = redux_post_meta('cl_redata',(int) codeless_get_post_id(), 'page_header_bool');
      $s_bool = redux_post_meta('cl_redata',(int) codeless_get_post_id(), 'slider_type');
...
function codeless_header_topnav_transparent(){
   global $cl_redata;

   if($cl_redata['top_navigation'] && $cl_redata['top_navigation_transparency']): ?>
        <div class="top_nav top_nav_transparency">
            <?php if(!$cl_redata['header_container_full'] || $cl_redata['header_style'] == 'header_7'): ?>
            <div class="container">
            <?php endif; ?>
                <div class="row-fluid">
                    <div class="span6">
...
function codeless_header_centered_logo($attr){
  global $cl_redata;

  $header_class = $cl_redata['header_style'];

  if($header_class == 'header_11'):
...
function codeless_header_tools(){
  global $cl_redata;
  
  if($cl_redata['header_style'] == 'header_5' || $cl_redata['show_search'] || class_exists('Woocommerce') || $cl_redata['extra_navigation']): ?>
...
function codeless_header_menu_button(){
  global $cl_redata;

  if($cl_redata['show_button']): ?>
    <!-- Header Button -->
...
function codeless_header_check_fullwidth_menu(){
  global $cl_redata;

  $header_class = $cl_redata['header_style'];

  if($header_class == 'header_6' || $header_class == 'header_10' || $header_class == 'header_12')
    return true;
...
function codeless_header_responsive_menu_icon(){
  global $cl_redata;

  if($cl_redata['responsive_menu_dropdown'] && $cl_redata['responsive_menu_style']=='normal'): ?>
...
function codeless_getPortfolioFields(){
  ...
  if(!empty($cl_redata['single_portfolio_custom_params']) ): 
      for($i = 0; $i < count($cl_redata['single_portfolio_custom_params']); $i++):
          $description_fields .= ($i+1).". ".esc_attr($cl_redata['single_portfolio_custom_params'][$i])."<br />  ";
      endfor;
  endif;
...
add_action('admin_head', 'remove_redux_adds');

function remove_redux_adds() {
  echo '<style>

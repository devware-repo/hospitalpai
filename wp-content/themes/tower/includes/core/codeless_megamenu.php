...

{
	...
	class custom_walker extends Walker {
		...
		function start_el(&$output, $item, $depth = 0, $args = array(),  $current_object_id = 0) {
			...
			$output .= $indent . '<li id="menu-item-'. $item->ID . '"' . $value . $class_names .' data-bg="'.$bg_img.'">';
...

	{
		...
		function start_el(&$output, $item, $depth = 0, $args = array() , $current_object_id = 0) {
			...
			if ( isset( $item->post_status ) && 'draft' == $item->post_status ) {
				...
				$title = sprintf( '%s'.__(' (Pending)', 'codeless'), $item->title );
			...
			<li id="menu-item-<?php echo $item_id; ?>" class="<?php echo $itemValue; echo implode(' ', $classes ); ?>">
				...
				<dl class="menu-item-bar">
					...
					<dt class="menu-item-handle">
						...
						<span class="item-controls">
							...
							<span class="item-type item-type-column"><?php _e('Mega Menu Column', 'codeless'); ?></span>

							<span class="item-type item-type-megalabel"><?php _e('Mega Menu', 'codeless'); ?></span>

							

							<a class="item-edit" id="edit-<?php echo $item_id; ?>" title="<?php _e('Edit Menu Item', 'codeless'); ?>" href="<?php

								echo ( isset( $_GET['edit-menu-item'] ) && $item_id == $_GET['edit-menu-item'] ) ? admin_url( 'nav-menus.php' ) : add_query_arg( 'edit-menu-item', $item_id, remove_query_arg( $removed_args, admin_url( 'nav-menus.php#menu-item-settings-' . $item_id ) ) );

							?>"><?php _e( 'Edit Menu Item' , 'codeless'); ?></a>
				...
				<div class="menu-item-settings" id="menu-item-settings-<?php echo $item_id; ?>">
					...
					<p class="description description-thin description-label label_desc_on_active">
						...
						<span class='default_label'><?php _e( 'Navigation Label', 'codeless' ); ?></span>

						<span class='mega_label'><?php _e( 'Title for the Mega Menu Column', 'codeless' ); ?></span>
					...
					<p class="description description-thin description-title">
						...
						<label for="edit-menu-item-attr-title-<?php echo $item_id; ?>">
							...
							<?php _e( 'Title Attribute', 'codeless' ); ?><br />
					...
					<p class="field-link-target description description-thin">
						...
						<label for="edit-menu-item-target-<?php echo $item_id; ?>">
							...
							<?php _e( 'link Target', 'codeless' ); ?><br />
							...
							<select id="edit-menu-item-target-<?php echo $item_id; ?>" class="widefat edit-menu-item-target" name="menu-item-target[<?php echo $item_id; ?>]">
								...
								<option value="" <?php selected( $item->target, ''); ?>><?php _e('Same window or tab', 'codeless'); ?></option>

								<option value="_blank" <?php selected( $item->target, '_blank'); ?>><?php _e('New window or tab', 'codeless'); ?></option>
					...
					<p class="field-css-classes description description-thin">
						...
						<label for="edit-menu-item-classes-<?php echo $item_id; ?>">
							...
							<?php _e( 'CSS Classes (optional)' , 'codeless'); ?><br />
					...
					<p class="field-xfn description description-thin">
						...
						<label for="edit-menu-item-xfn-<?php echo $item_id; ?>">
							...
							<?php _e( 'link Relationship (XFN)', 'codeless' ); ?><br />
					...
					<p class="field-description description description-wide">
						...
						<label for="edit-menu-item-description-<?php echo $item_id; ?>">
							...
							<?php _e( 'Description', 'codeless' ); ?><br />
					...
					<div class='codeless_custom_menu_options'>
						...
						<p class="description description-wide codeless_background_image codeless_custom_mega_menu codeless_custom_mega_menu_d1">
							...
							<label for="edit-<?php echo $key.'-'.$item_id; ?>">
								...
								<input type="text" id="edit-<?php echo $key.'-'.$item_id; ?>" class=" <?php echo $key; ?>" name="<?php echo $key . "[". $item_id ."]";?>" value="<?php echo $value; ?>" /><?php echo $title; ?>
					...
					<div class="menu-item-actions description-wide submitbox">
						...
						<?php if( 'custom' != $item->type ) : ?>
							...
							<p class="link-to-original">
								...
								<?php printf( __('Original: %s', 'codeless'), '<a href="' . esc_attr( $item->url ) . '">' . esc_html( $original_title ) . '</a>' ); ?>
						...
						); ?>"><?php _e('Remove', 'codeless'); ?></a> <span class="meta-sep"> | </span> <a class="item-cancel submitcancel" id="cancel-<?php echo $item_id; ?>" href="<?php	echo add_query_arg( array('edit-menu-item' => $item_id, 'cancel' => time()), remove_query_arg( $removed_args, admin_url( 'nav-menus.php' ) ) );

							?>#menu-item-settings-<?php echo $item_id; ?>"><?php _e('Cancel', 'codeless'); ?></a>

    ...
    function codeless_sc_lightbox($atts, $content = null, $shortcodename=""){
        ...
        extract(shortcode_atts(array("image_link" => "", 'video' => ''), $atts));
            ...
            if($video != ''){
                    ...
                    $output .= '<a class="lightbox-gallery" href="'.esc_url($image_link).'" title="">';
...
if(!function_exists('codeless_sc_icon_text')){
    ...
    function codeless_sc_icon_text($atts, $content = null, $shortcodename=""){
        ...
        $output = ''; 
        $output .= '<div class="icon_text"><i class="'.esc_attr($icon).'"></i><span>'.$content.'</span></div>';   
...
if(!function_exists('codeless_sc_shortcode_button')){
    ...
    function codeless_sc_shortcode_button($atts, $content = null, $shortcodename=""){
         ...
         extract(shortcode_atts(array("link" => "", "icon" => "", "type" => "", "target"=>"", "align" => "", "bordercolor" => "", "bordercolor_hover", "color"=>"", "bg_color_hover"=> "","font_color"=>"", "icon_color" => "", "icon_color_hover" =>"", "font_color_hover" =>""), $atts));
            $border_c = $icon_c = $border_c_h = $icon_c_h = '';
            ...
            if(!empty($icon_color_hover))
                ...
                $output .= '<div class="wpb_content_element button shortcode_button">';
...
if(!function_exists('codeless_sc_contact_info')){
    ...
    function codeless_sc_contact_info($atts, $content = null, $shortcodename=""){
        ...
        extract(shortcode_atts(array("tel" => "", 'addr' => '', 'email' => '', 'bbm' => '', 'light' => ''), $atts));
        

        $light_class = '';
        if($light == 'yes')
            $light_class = 'light';
        $output = '<div class="contact_information '.$light_class.'">';
...
if(!function_exists('codeless_sc_font_style')){

    function codeless_sc_font_style($atts, $content = null, $shortcodename=""){

        extract(shortcode_atts(array("fontsize" => "", 'fontweight' => '', 'color' => '', 'lineheight' => '', 'underline' => 'false'), $atts));
        ...
        $extra_class = '';
        if($underline == 'true')
            $extra_class .= 'underline_text';

        $output = '<span class="font_shortcode '.$extra_class.'" style="'.$style.'">';
            $output .= $content;
        $output .= '</span>';  
    ...
    add_shortcode('font_style', 'codeless_sc_font_style');
}

if(!function_exists('codeless_sc_simple_counter')){

    function codeless_sc_simple_counter($atts, $content = null, $shortcodename=""){

        extract(shortcode_atts(array("number" => "", 'text' => ''), $atts));
       
        wp_enqueue_style( 'odometer-theme-minimal' );
        wp_enqueue_script('odometer.min');
        $output .= '<div class="simple_counter">';

            $output .= '<div class="count_to animate_onoffset">';
                $output .= '<div class="odometer" data-number="'.(int) $number.'" data-duration="2000"></div>';
             $output .= '</div>';
             $output .= '<h3>'.$text.'</h3>';
        $output .= '</div>';  
    ...
    add_shortcode('simple_counter', 'codeless_sc_simple_counter');
}

if(!function_exists('codeless_revolution_vc')){

    function codeless_revolution_vc($atts, $content = null, $shortcodename=""){
    ...
    add_shortcode('revolution_vc', 'codeless_revolution_vc');
}

...
require_once get_template_directory().'/functions-tower.php';
...
function codeless_images_sizes(){
    ...
    //add_image_size( 'port3_3', 265, 550, true );
...
function codeless_register_global_styles(){
    ...
    $p_id = codeless_get_post_id();
    if(isset($p_id) && !empty($p_id))
        if( redux_post_meta('cl_redata',(int) $p_id, 'fullscreen_post_style' ) || $cl_redata['fullscreen_sections_active'] )
            wp_enqueue_style('fullscreen_post_css');


    if($p_id == $cl_redata['blogpage'] && $cl_redata['blog_style'] == 'fullscreen'){
...
function codeless_register_global_scripts(){
    ...
    $p_id = codeless_get_post_id();
    if(isset($p_id) && !empty($p_id))
        if( redux_post_meta('cl_redata',(int) $p_id, 'fullscreen_post_style' ) )   
            wp_enqueue_script('fullscreen_post');

    if( $cl_redata['codeless_page_transition']){
        wp_enqueue_script('jquery.animsition.min');
    }

...
class CodelessImportCore{
	...
	public function __construct(){
        ...
        function codeless_import_content($file, $attachments, $zip = 'none'){
			ob_start();
			if (!class_exists('WP_Importer')) {
				...
				if($attachments && $zip != 'none' && !empty($zip)){
					$file = $file.'_new';
					$new_tech = true;
				}
				...
				if(is_wp_error($value)){
					...
					if($new_tech){
						$this->deleteDirectory(get_template_directory().'/downloads');
						unlink(get_template_directory() . '/includes/dummy_data/'.$this->type.'/' . $file.'.xml');
					}
					return 2;
				}
				else {
		...
		function modifyXml($file){
			$path = get_template_directory() . '/includes/dummy_data/demos/' . $file.'.xml';
			// load the document
			...
			foreach($items as $item){
				...
				$wp_children->attachment_url = str_replace('http://codeless.co/tower/default_new/wp-content/uploads/', get_template_directory_uri().'/downloads/uploads/', $wp_children->attachment_url);

			}

			// save the updated document
			if($channel->asXML(get_template_directory() . '/includes/dummy_data/demos/' . $file.'_new.xml'))
				return true;
...
/* ------------------------------------------------ AJAX -----------------------------------------------------------------*/
        ...
        function ajax_codeless_import_content()
		{
			...
			if($attachments && $zip != 'none'){
				if($codelessCore->prepareFile($_POST['demo'], $zip) ){
					if($codelessCore->modifyXml($path.$_POST['xml']))
						$status = $codelessCore->codeless_import_content($path.$_POST['xml'], $attachments, $zip);
				}

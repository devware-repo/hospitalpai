...
extract( $atts );
        ...
        if($rows == 1){
            ...
            if($mode == 'masonry' && (int)$columns == 4 && $rows == 4)
                $coe = 14;
        ...
        if($from_where == 'all_cat'){
           ...
           if(is_object($category))
            ...
            $output .= '<section id="portfolio-preview-items" class="row mode_'.$mode.' '.esc_attr($space).' '.esc_attr($grid).'" data-cols="'.esc_attr($columns).'">';
